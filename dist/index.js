 ...
 */
import { __, curry, flatten, forEachObjIndexed, identity, ifElse, includes, is, join, keys, map, objOf, of, pipe, reject, replace, test, unless } from 'ramda';
 ...
 */
const $value = ifElse(Array.isArray, pipe(map(quotize), join(',')), quotize);
 ...
 */
const $query = curry((valueFormatter, field, operation, value) => alt('', `${$operation(operation)}(${field},${valueFormatter(value)})`, isNilOrEmpty(value)));
 ...
 */
const qOr = (values) => values.reduce((acc, v) => alt(acc, alt(`(${v})`, `(${acc}|${v})`, isNilOrEmpty(acc)), isNilOrEmpty(v)));
 ...
 */
const qWrap = (operation, value, formatter = identity) => alt('', `${$operation(operation)}(${formatter(value)})`, isNilOrEmpty(value));
 ...
 */
const qAnd = pipe(reject(isNilOrEmpty), join('&'));
/** Prepares value to Limit RQL expression
 *
 * @type {Function}
 * @param {Object} limitParams - limit params (count and start)
 * @returns {String}
 *
 * @summary Array -> String
 * @example ({start: 10, count: 20}) -> 'limit(10,20)'
 */
const qLimit = ({ start = 0, count = 1000 }) => `limit(${start},${count})`;
 ...
 */
const toWrappedQueries = (field, operation, rqlFilters) => pipe(unless(is(Array), of), reject(isNilOrEmpty), map((rqlFilter) => map((subKey) => {
    const subRql = objOf(subKey, rqlFilter[subKey]);
    const subQuery = rqlToQuery(subRql, field);
    return qWrap(operation, subQuery);
}, keys(rqlFilter))), flatten)(rqlFilters);
 ...
 */
const toWrappedQuery = pipe(rql, unless(isNilOrEmpty, (v) => `(${v})`));
...
export function rqlToQuery(rqlExp, field) {
    const rqlFilter = [];
    const toSubQueries = map(pipe(objOf(field), toWrappedQuery));
    Object.entries(rqlExp).map((value, operation) => {
 ...
 */
export default function rql(rqlObj) {
    const result = [];
